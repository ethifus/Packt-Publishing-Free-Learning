...
[PEX](https://pex.readthedocs.io/en/stable/whatispex.html) files are
self-contained executable Python virtual environments. You can create one with
this packages using command:

    pip install pex
    pex . -e packt_publishing.claim_free_ebook:main packt_claim_free_ebook.pex

You can then use command resulting excecutable file `packt_claim_free_ebook.pex`
instead of `python packtPublishingFreeEbook.py ...` (without need of installing
anything additional).
